...
class FlexibleDesignerField(serializers.PrimaryKeyRelatedField):
    ...
    def to_internal_value(self, data):
        ...
        if isinstance(data, str) and data.strip():
            designer = (
                Designer.objects.filter(brand_name__iexact=data.strip()).first() or
                Designer.objects.filter(designer_name__iexact=data.strip()).first() or
                Designer.objects.filter(designer_code__iexact=data.strip()).first()
            )
            if designer:
                return designer
            raise serializers.ValidationError(f"Designer '{data}' not found.")
...
class FlexibleReturnPolicyField(serializers.CharField):
    def to_internal_value(self, data):
        if data is True or str(data).strip().lower() in ["true", "returnable", "yes"]:
            return Product.ReturnPolicy.RETURNABLE
        if data is False or str(data).strip().lower() in ["false", "final_sale", "final sale", "no"]:
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    designer_name = serializers.ReadOnlyField(source="designer.designer_name")
    designer_brand = serializers.ReadOnlyField(source="designer.brand_name")
    designer_code = serializers.ReadOnlyField(source="designer.designer_code")

    fulfilment_location = FlexibleLocationField(required=False, default=Product.FulfilmentLocation.MUMBAI_FC)
    size = FlexibleSizeField(required=False, default=Product.Size.FREE)
    return_policy = FlexibleReturnPolicyField(required=False, default=Product.ReturnPolicy.RETURNABLE)
    ...
    def to_internal_value(self, data):
        ...
        if "returnable" in data_copy and "return_policy" not in data_copy:
            data_copy["return_policy"] = "RETURNABLE" if data_copy["returnable"] else "FINAL_SALE"
        if "designerId" in data_copy and "designer" not in data_copy:
            data_copy["designer"] = data_copy["designerId"]
        
        # CamelCase to snake_case aliases for metadata fields
    ...
    def to_representation(self, instance):
        ...
        # Expose camelCase aliases expected by frontend
        ...
        res["returnable"] = instance.return_policy == Product.ReturnPolicy.RETURNABLE
        ...
        res["price"] = float(instance.selling_price) if instance.selling_price else 0.0
        return res
    ...
    def validate(self, attrs):
        mrp = attrs.get("mrp", getattr(self.instance, "mrp", Decimal("0")))
        selling_price = attrs.get("selling_price", getattr(self.instance, "selling_price", Decimal("0")))
        ...
        if mrp > Decimal("0"):
            calc_disc = ((Decimal(str(mrp)) - Decimal(str(selling_price))) / Decimal(str(mrp))) * Decimal("100")
