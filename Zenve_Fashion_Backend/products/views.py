    ...
    def get(self, request):
        ...
        if designer:
            ...
            if designer_val.isdigit():
                products = products.filter(designer_id=int(designer_val))
            else:
                products = products.filter(
                    models.Q(designer__brand_name__iexact=designer_val) |
                    models.Q(designer__designer_name__iexact=designer_val) |
                    models.Q(designer__designer_code__iexact=designer_val)
                )
        if category:
            products = products.filter(category__iexact=category.strip())
        if status_filter:
            products = products.filter(status__iexact=status_filter.strip())
        is_live = request.query_params.get("is_live")
        if is_live is not None:
            is_live_bool = str(is_live).strip().lower() in ["true", "1", "yes"]
...
class ProductStockAdjustmentAPIView(APIView):
    ...
    Body:
    {
        "action": "receive" | "damage" | "quarantine" | "release_quarantine",
        "quantity": int
    }
    """
    permission_classes = [AllowAny]
    ...
    def post(self, request, pk):
        ...
        if action == "receive":
            ...
            qty_to_release = min(qty, product.quarantined_quantity)
            ...
            qty_to_release = min(qty, product.reserved_quantity)
