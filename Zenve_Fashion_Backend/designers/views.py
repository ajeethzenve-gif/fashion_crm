...
class DesignerListAPIView(APIView):
    ...
    def get(self, request):
        designers = Designer.objects.all().order_by("-created_at")
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(
                DesignerSerializer(designer).data,
...
class DesignerDetailAPIView(APIView):
    ...
    def get_object(self, designer_id):
        try:
            return Designer.objects.get(id=designer_id)
    ...
    def get(self, request, designer_id):
        ...
        designer = self.get_object(designer_id)
    ...
    def patch(self, request, designer_id):
        import time
        ...
        # Support adding a new follow-up task directly
        if "add_task" in data:
            task = data.pop("add_task")
            tasks = list(designer.follow_up_tasks or [])
            ...
            designer.follow_up_tasks = tasks
            designer.save(update_fields=["follow_up_tasks"])

        # Support toggling a task's completion
        if "toggle_task_id" in data:
            tid = data.pop("toggle_task_id")
            tasks = list(designer.follow_up_tasks or [])
            for t in tasks:
        ...
        # If lost_reason is provided, set stage to REJECTED/INACTIVE if not specified
        if "lost_reason" in data and data["lost_reason"]:
    ...
    def delete(self, request, designer_id):
    ...
    def get(self, request, designer_id):
        ...
        return Response({
            "designer": {
                ...
                "name": designer.owner_name or designer.designer_name,
                "contact": designer.email or designer.phone or "—",
                "city": designer.city,
                "stage": designer.stage,
                "kyc": designer.kyc_status == "VERIFIED",
                "gst": designer.gst_number or "—",
                "contractEnds": str(designer.contract_end_date) if designer.contract_end_date else "—",
                "takeRate": float(designer.take_rate)
            },
            "kpis": {
